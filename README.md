...
- **Mostrar/ocultar senha**: todo campo de senha do app (entrar, cadastrar e redefinir senha de outra pessoa) tem um ícone de olhinho (👁️) do lado do campo — clicando nele, dá pra ver a senha digitada em texto normal, em vez dos pontinhos, pra conferir se digitou certo antes de enviar.
...
- **Editar mensagem enviada no chat da turma**: Professora Regente, Professora Auxiliar, Estagiária e qualquer pessoa da Direção podem clicar no ícone ✏️ na própria mensagem para corrigi-la (aparece um "(editado)" do lado do horário, pra ficar claro que o texto mudou). Responsável e Cozinha não têm essa opção. Não tem limite de tempo nem histórico das versões antigas.
- **Fixar mensagem no chat da turma**: Professora Regente e qualquer pessoa da Direção podem clicar no ícone 📌 numa mensagem para fixá-la (aparece um aviso "📌 Fixado por Fulana" nela). Mensagem fixada **não é apagada pela limpeza automática de 5 dias** — só some se quem a enviou apagar manualmente (ícone 🗑, como já funcionava). Clicar em 📍 desfixa.
...
- **Financeiro: só o saldo aparece de cara**: a aba "Financeiro" agora mostra só o **saldo geral** e uma lista dos meses com lançamento; clicar num mês abre o detalhe (receitas, despesas e a lista de lançamentos daquele mês). Diretora, Gestor e Secretária lançam; qualquer pessoa logada pode visualizar (transparência com as famílias); só Diretora e Gestor podem excluir lançamentos.
- **Despesa parcelada (crediário)**: ao lançar uma despesa, dá pra marcar "Despesa parcelada (crediário)" e informar o valor total, o número de parcelas e a data de vencimento da 1ª — o app cria uma parcela por mês automaticamente (mesmo valor em cada uma, com o arredondamento de centavos ficando na última). **Cada parcela só entra no saldo/relatório depois de "quitada"**: isso acontece sozinho quando a data da parcela chega, ou antes disso se Diretora/Gestor/Secretária clicar em "quitar agora" na lista de "Despesas parceladas a vencer" (que aparece no topo da aba Financeiro enquanto houver parcela pendente).
...
- **Enquete: agora tem dois jeitos de criar, cada um com sua função**:
  - **Enquete dentro do chat da turma (📊 Enquete)**: voltou o botão "📊 Enquete" no topo do chat de cada turma, visível para Professora Regente, Professora Auxiliar, Estagiária e Direção/Gestor. A professora que já está ali dentro cria a pergunta (até 8 opções) sem precisar escolher turma nenhuma — é só pra aquela turma mesmo, aparece direto no chat, e todo mundo pode votar e **trocar o voto** quando quiser (não é obrigatório votar, é só uma consulta rápida do dia a dia).
  - **Enquete avulsa (aba "Enquetes")**: continua existindo, mas agora é **só para Direção e Gestor** (Diretora, Coordenadora Pedagógica, Secretária, Gestor) — porque essa é a que escolhe entre enviar para **todo mundo** ou só para **uma turma específica** da creche toda, e por isso precisa da lista completa de turmas. Funciona igual a um recado: aparece em tela cheia assim que a pessoa abre o app e só some depois que ela **vota** (voto vale como confirmação, não dá pra trocar depois). Quem criou acompanha em tempo real quantos votos cada opção tem e quem ainda falta votar.
...
| Lançar receita/despesa (inclusive parcelada) | ❌ | ❌ | ❌ | ❌ | ❌ | ✅ | ❌ | ✅ | ✅ |
| Marcar parcela como paga antes do vencimento | ❌ | ❌ | ❌ | ❌ | ❌ | ✅ | ❌ | ✅ | ✅ |
...
| Editar a própria mensagem no chat da turma | ❌ | ✅ | ✅ | ✅ | ❌ | ✅ | ✅ | ✅ | ✅ |
| Fixar/desafixar mensagem no chat da turma | ❌ | ❌ | ✅ | ❌ | ❌ | ✅ | ✅ | ✅ | ✅ |
| Criar enquete dentro do chat da turma | ❌ | ✅ | ✅ | ✅ | ❌ | ✅ | ✅ | ✅ | ✅ |
| Criar enquete avulsa (aba "Enquetes") | ❌ | ❌ | ❌ | ❌ | ❌ | ✅ | ✅ | ✅ | ✅ |
| Votar em enquete                | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
...
- Chat da turma: dono da mensagem, professora regente ou qualquer pessoa da direção podem apagar na hora; além disso, **toda mensagem de turma soma 5 dias e é apagada automaticamente para sempre**, mesmo que ninguém peça (veja a seção "🗑️ Retenção de mensagens") — **exceto mensagem fixada (📌)**, que fica isenta dessa limpeza automática e só some se quem a enviou apagar manualmente.
